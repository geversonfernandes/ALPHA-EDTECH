...
).then((response) => response.json().then((data) => displayDistricts(data)));
...
function fetchCities(){
    return new Promise((resolve, reject) => {
        ...
        fetch(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${district}/municipios`)
        .then(response => {
            if(response.status === 200){
                return response.json();
            };
            throw 'Erro: status dif. de 200'
        })
        .then((data2) => {
            resolve(0);
            displayCities(data2);
...
function fetchWeather() {
  return new Promise((resolve, reject) => {
    ...
    fetch(`https://apiprevmet3.inmet.gov.br/previsao/${city}`)
      .then((response) => {
        if (response.status === 200) {
          return response.json();
        }
        return Promise.reject("Failed to request data");
      })
      .then((data3) => resolve(attTable(data3)))
      .catch((err) => {
        reject(err);
      });
  });
